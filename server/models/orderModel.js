...
class Order {
    ...
    async getOne(id, userId = null) {
        ...
        if (!order) throw new Error('Заказ не найден в БД')
    ...
    async create(data) {
        ...
        const order = await OrderMapping.create({name, email, phone, address, comment, amount, userId})
