...
import { fetchOneDevice, updateDevice, fetchTypes, fetchBrands } from '../http/catalogAPI.js'
...
const defaultValue = {name: '', price: '', type: '', brand: ''}
const defaultValid = {name: null, price: null, type: null, brand: null}
...
const updateProperties = async (properties, deviceId) => {
    for (const prop of properties) {
        ...
        if (prop.append && !empty) {
            try {
                await createProperty(deviceId, prop)
            } catch(error) {
                alert(error.response.data.message)
            }
            continue
        ...
        if (prop.change && !prop.remove) {
            try {
                await updateProperty(deviceId, prop.id, prop)
            } catch(error) {
                alert(error.response.data.message)
            }
            continue
        ...
        if (prop.remove) {
            try {
                await deleteProperty(deviceId, prop.id)
            } catch(error) {
                alert(error.response.data.message)
            }
            continue
...
const UpdateDevice = (props) => {
    ...
    const handleSubmit = async (event) => {
        ...
        if (correct.name && correct.price && correct.type && correct.brand) {
